...
| F-15 | 出荷指示削除 | 明細ごと削除。引当済なら在庫を戻す。出荷済は削除不可 | `ShipmentService#delete` |
...
| 登録済(10) / 取消(90) | 削除 | (レコード削除) | なし | ― |
| 引当済(20) | 削除 | (レコード削除) | 明細の数量だけ**加算**(戻し) | ― |
| 出荷済(30) | 削除 | ― | ― | エラー「出荷済の出荷指示は削除できません」 |

> **出荷済を削除できない理由**: 出荷済は出荷実績として残す必要があるため。
> 誤って登録した伝票を取り下げたい場合は、出荷確定の前に取消(F-14)を行う。
...
- **出荷済(30)は取消・削除ともに不可**。出荷実績として残す。
- 取消済(90)の再取消は不可だが、削除は可能。
...
| E-14 | 出荷済の出荷指示は削除できません | 出荷済の削除 | `ShipmentService#delete` |
